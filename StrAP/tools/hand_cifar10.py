...
def find_cifar_python_root(root: str) -> str:
    ...
    candidates = [
        root,
        os.path.join(root, "cifar-10-python"),
        os.path.join(root, "data"),
        os.path.join(root, "data", "cifar-10"),
        os.path.join(root, "data", "cifar-10", "cifar-10-python"),
    ]

    print(f"[INFO] Searching for cifar-10-python in root: {root}")
    for c in candidates:
        print(f"[INFO] Checking candidate path: {c}")
        if os.path.isdir(c) and any(f"data_batch_{i}" in os.listdir(c) for i in range(1, 6)) and "test_batch" in os.listdir(c):
            return c

    raise FileNotFoundError(
        f"Cannot find 'cifar-10-python' (with data_batch_1 to data_batch_5 and test_batch) under root={root!r}."
    )


def load_cifar10_batch(python_root: str, batch_name: str) -> Tuple[np.ndarray, List[int], List[str]]:
    fpath = os.path.join(python_root, batch_name)
    if not os.path.isfile(fpath):
        raise FileNotFoundError(f"Batch file {batch_name!r} not found in {python_root!r}")
    ...
    data = entry["data"]
    labels = entry["labels"]
    filenames = entry.get("filenames", [f"{batch_name}_{i}" for i in range(len(labels))])
...
def load_class_map(python_root: str) -> Dict[str, int]:
    meta_path = os.path.join(python_root, "batches.meta")
...
def main():
    ap = argparse.ArgumentParser(description="Convert CIFAR-10 (cifar-10-python) to federated classification JSON.")
    ap.add_argument("--root", required=True, help="Root directory containing cifar-10-python/ (can have data/cifar-10 outside)")
    ap.add_argument("--alpha", type=float, default=0.3, help="Dirichlet alpha value (default: 0.3)")
    ap.add_argument("--out", default="./datasets/data_cache/cifar10_train.json",
                    help="Federated training JSON output path")
    ap.add_argument("--val_out", default="./datasets/data_cache/cifar10_val.json",
    ...
    print(f"[INFO] cifar-10-python root: {cifar_py_root}")
    ...
    train_imgs, train_labels, _ = load_cifar10_batch(cifar_py_root, "data_batch_1")
    for i in range(2, 6):
        batch_imgs, batch_labels, _ = load_cifar10_batch(cifar_py_root, f"data_batch_{i}")
        train_imgs = np.concatenate([train_imgs, batch_imgs], axis=0)
        train_labels.extend(batch_labels)

    test_imgs, test_labels, _ = load_cifar10_batch(cifar_py_root, "test_batch")
    ...
    federated_train_data_path = f"./datasets/data_cache/cifar10_federated_{alpha_tag}_train.json"
    federated_val_data_path = f"./datasets/data_cache/cifar10_federated_{alpha_tag}_val.json"
